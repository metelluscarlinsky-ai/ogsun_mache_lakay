...
app.post('/api/admin/products', verifyAdmin, (req, res) => {
  ...
  const np = { id: products.length + 1, name, description: description || '', price: parseFloat(price), image_url: image_url || 'logo.png', category_id: parseInt(category_id), created_at: new Date().toISOString() };
  products.push(np);
...
app.post('/api/admin/upload', verifyAdmin, upload.single('image'), async (req, res) => {
  try {
    ...
    const uploadUrl = `https://tbgltnltmbeobfsctixt.supabase.co/storage/v1/object/product-images/${fileName}`;
    const response = await fetch(uploadUrl, {
      method: 'POST',
      headers: {
        'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRiZ2x0bmx0bWJlb2Jmc2N0aXh0Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NDc0NzcxMywiZXhwIjoyMTAwMzIzNzEzfQ.HvP5aRKwjIXbJIaUdIT4_04nun9hNv6tD-UClQ-G-cA',
    ...
    if (!response.ok) {
      ...
      return res.status(500).json({ error: 'Upload echwe: ' + response.status });
    }

    const imageUrl = `https://tbgltnltmbeobfsctixt.supabase.co/storage/v1/object/public/product-images/${fileName}`;
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('📸 Upload: Supabase Storage (API REST)');
  console.log('💾 Done: Fichye JSON lokal');
});
